...
sgMail.setApiKey(
  "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"
);
...
const filterAdminBlacklist = async (term) => {
  const blacklistedWords = await _getMetaFromTitle("blacklist")
    ...
    .then((content) => content.split(", "));
  return term.replace(
    new RegExp("\b" + blacklistedWords.join("\b|\b") + "\b", "gi"),
    ""
  );
};
...
const getSongsFromTerm = (term, cb) => {
  ...
  getTermEquivalences(term)
    ...
    .then(([allSongs, terms]) => {
      ...
      // Remove duplicates
      songs = songs.filter(
        (song, i, songs) => i === songs.findIndex((s) => s.title === song.title)
      );
      ...
      songs = songs.filter((s) => {
        ...
        const searchKeywords = [
          ...s.primary_keywords,
          ...s.instruments,
          ...s.searchable_keywords,
        ];

        return anyContainsTerms(
          [s.title, s.genre.join(" "), searchKeywords.join(" ")],
          terms
        );
      });
  ...
  Song.find()
    ...
    .then((songs) => {
      return Meta.find({})
        ...
        .then((meta) => {
          return ActivityLog.find({}).then((activityLog) => {
